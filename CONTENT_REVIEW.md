...
## 2026-07-25 애드센스 재검토 전 개선

### 확인한 주요 원인

- 라이브 `/ads.txt`가 광고 판매자 텍스트가 아니라 빈 블로그 HTML을 `200`으로 반환해 soft 404 상태였습니다.
- 공개 7편 중 3편이 RSS 기사에서 출발한 짧은 연구 요약이어서 자동 생성·저가치 콘텐츠로 판단될 위험이 있었습니다.
- 개인 운영 사이트인데 작성자와 검토자를 모두 `KidneyLife 자료 편집부`로 표시해 실제 책임 주체와 의료 감수 범위가 불명확했습니다.
- 글마다 AI 보조 여부와 사람이 확인한 범위가 사용자 화면에 표시되지 않았습니다.
- 사이트의 기본 URL 예시와 일부 fallback이 실제 주소가 아닌 `kidney-life.vercel.app`을 가리켰습니다.

### 적용한 개선

- `frontend/public/ads.txt`에 게시자 ID를 추가하고 미들웨어에서 `/ads.txt`를 정적 파일로 통과시켰습니다.
- 지원하지 않는 `[lang]` 경로를 404 처리하고 기본 canonical·robots·sitemap 주소를 `https://blog-kidney.vercel.app`으로 통일했습니다.
- 광고 스크립트를 전역 레이아웃에서 제거해 홈과 글 상세에서만 조건부 로드되도록 범위를 줄였습니다.
- RSS 기반 연구 요약 3편을 `draft`로 전환했습니다. 수집기는 계속 새 자료를 초안으로만 저장합니다.
- 작성자를 `KidneyLife 운영자`로 통일하고 개인 운영·비의료인·비의학 감수임을 소개, 글 상단, 작성자 정보와 구조화 데이터에 표시했습니다.
- 공개 글은 `creation_note`가 있어야 발행되도록 스키마를 강화했습니다. AI 보조 초안 사용과 운영자의 원문 대조·재작성 범위를 글마다 공개합니다.
- 집 혈압 기록, 부종·체중 기록, 신장이식 평가 준비, 식사 상담 준비 안내 4편을 새로 작성했습니다.
- 검사 추세, 약 목록, 진료 질문, 증상·체중, 투석 상담을 인쇄할 수 있는 `/ko/resources` 페이지를 추가하고 관련 글에서 연결했습니다.
- 새 글 대표 이미지는 외부 사진을 복제하지 않고 자체 SVG로 제작했습니다.
...
- 검토 대기 초안: 5편
- 제외: 3편
- `python backend/scripts/build_data.py`: 공개 글 8편 생성
- `python backend/scripts/audit_content.py --strict`: 오류 0, 경고 0
- 대표 이미지 8개 경로, 작성 과정 필드와 SVG 8개 XML 문법 확인 완료
- `git diff --check`: 공백 오류 없음(Windows 줄바꿈 안내만 출력)
- 사용자 요청에 따라 Next.js 빌드, 개발 서버, 브라우저 렌더링과 스크린샷 검사는 실행하지 않았습니다.

배포 뒤에는 `https://blog-kidney.vercel.app/ads.txt`가 `text/plain`으로 한 줄만 반환하는지, Search Console에서 sitemap과 색인 제외 사유가 무엇인지, AdSense 자동 광고가 정책·소개·기록 양식 페이지에 노출되지 않는지를 운영자가 확인해야 합니다.
